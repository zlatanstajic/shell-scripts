...
4. **Regenerate the flags/usage reference.** The flags/usage reference (each
   ...
   include. `bash src/scripts/gen-docs.sh --check` reports drift; it is a hard gate
   in CI and an advisory (non-blocking) step in the pre-commit hook.
5. **Test before submitting.** Run the affected script(s) with
   ...
   Unix-like system. Then run the three checks CI enforces, all of which are
   hard gates:
